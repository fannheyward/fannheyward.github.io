...
编译能通过，不能运行，这个报错生生折腾了我两个小时，起初以为是环境变量没设好，改了N次，仍然是报错，气得半死。Google的时候说Java严格区分大小写，就留心了一下，果然是这个问题。类名是 class Hello，大写，文件名却是 hello.java，小写，编译的时候javac不区分大小写，所以编译通过，生成 Hello.class 文件，运行的时候却是java hello，与类名不符，进而 could not find the main class，所以报错。
